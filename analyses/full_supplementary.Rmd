...
For the Paracou site, we used the plots 1 to 15 of the Paracou site. We removed all trees of undetermined species and all measures of the years before the perturbations were performed and some of the plots (called the biodiversity plots) were included in the survey (1985-1991).
...
For the Paracou site, after removing `r nrow(Paracou_no_mean[which(Paracou_no_mean$Sp=="Indet_sp"),])` growth estimates for `r length(unique(Paracou_no_mean[which(Paracou_no_mean$Sp=="Indet_sp"),]$idTree))` trees of indeterminate species, this leads to a dataset with `r nrow(Paracou_no_mean[which(Paracou_no_mean$Sp!="Indet_sp"),])` growth estimates for `r length(unique(Paracou_no_mean[which(Paracou_no_mean$Sp!="Indet_sp"),]$idTree))` trees of `r length(unique(Paracou_no_mean[which(Paracou_no_mean$Sp!="Indet_sp"),]$Sp))` species.

For the Uppangala site, this leads to a dataset with `r nrow(Uppangala_no_mean)` growth estimates for `r length(unique(Uppangala_no_mean$TreeID))` trees of `r length(unique(Uppangala_no_mean$SpCode))` species.

For the BCI site, this leads to a dataset with `r nrow(BCI_no_mean)` growth estimates for `r length(unique(BCI_no_mean$treeID))` trees of `r length(unique(BCI_no_mean$sp))` species.
...
To quantify the relative importance of intraspecific and interspecific variability in annual tree growth for each site, we built a model incorporating a species random effect and an individual random effect on the intercept. In order to compare to classic growth models, and estimate an intraspecific variability within diameter classes, we also added an intercept and a diameter fixed effect. This model was fitted in a hierarchical Bayesian framework using the brms package [@burkner_brms_2017 ; @burkner_advanced_2018] with 10,000 iterations, a warming period of 5,000 iterations and a thinning of 5. We obtained 1,000 estimates per parameter.
...
kableExtra::kbl(
  ...
  caption = "Residual variance partitioning of annual tree growth in three tropical forest sites. Shown are the mean posterior estimates of the growth models fitted for each site as well as the corresponding estimation error. Annual tree growth was modelled with an intercept, a fixed effect for the diameter, and random effects for the species and the individuals, allowing to compare the variance associated to species and to individuals, V being the residual variance."
  ) %>%
...
```{r PlotsVarianceProportion, fig.show="hold", out.width="33%", fig.cap="Variance partitioning of individual growth in three tropical forest sites (see Table above)."}
...
Overall, a large part of the unexplained variance in  annual tree growth after accounting for the effect of diameter was imputable to individual effects in the three sites, showing a high IV in growth in this tropical forests (Figure \@ref(fig:PlotsVarianceProportion)).

## Moran's I analysis

In order to test if individual tree growth was spatially autocorrelated within each site, we performed a Moran’s I test on mean annual tree growth for each species with more than 5 individuals. For very abundant species (with more than 3,000 individuals), we sampled 3,000 individuals using a uniform probability. Moran’s I test function was re-written from the ape package’s Moran.I function with a one-tailed test [@paradis_ape_2019] in order to be able to select the pairs of neighbours that are less than 100-m apart and thus assess local spatial autocorrelation.
...
kableExtra::kbl(Results_moran, booktabs = T, caption = "Spatial autocorrelation of the mean growth of conspecific individuals in three tropical forest sites. Shown are the proportion of species, and of corresponding individuals, in percent, for which mean individual tree growth is significantly positively spatially autocorrelated. The spatial autocorrelation of mean individual tree growth was tested using Moran’s I index.")%>%
...
Although the three forest sites are located in very different areas across the tropics, with contrasting biogeographical history, climate, regime of perturbations and topography, they all showed a substantial spatial autocorrelation of mean tree growth, which supports the generality of our results. The magnitude of spatial autocorrelation varied across the three sites however, with the Paracou site presenting the biggest proportion and the Uppangala site the lowest, which can be related to each site specificities. The plots of the Paracou site were subjected to disturbance treatments that created numerous large gaps [@molino_tree_2001]. This resulted in a high heterogeneity and spatial structuration of the environment. When restricting our analysis to undisturbed control plots (see \@ref(undisturbed) below), the proportion of detected spatial autocorrelation in conspecific individual tree growth drops from `r round(as.numeric(Results_moran[2,]$Significant))`% to `r round(as.numeric(Results_moran[6,]$Significant))`% individuals, supporting the hypothesis that the more important spatial autocorrelation in the Paracou site is due to the stronger spatial structuration of the micro-environment associated with disturbance treatments. Conversely, the disturbance regime in the BCI site is known to produce frequent small gaps [@hubbell_light-gap_1999], while the mature forest in the Uppangala site has been reported as barely disturbed [@pelissier_tree_2011]. We acknowledge that the hilly topography of the Uppangala site could make our estimates of distances among individuals based on pairs of tree coordinates provided by field inventory less accurate than in the other two flatter sites.


## Is variance within conspecifics inferior to variance within heterospecifics ?

In order to test if the performance of conspecifics was locally more similar than that of heterospecifics in the three tropical forests, for each species with more than five individuals and more than five heterospecific neighbours within 100-m radii around these individuals, we computed the semivariances of the mean annual growth of individuals that are less than 100-m apart, considering conspecifics only on the one hand, and heterospecifics on the other hand. In the first case, semivariance was estimated as the mean of the squared difference in mean tree growth for all pairs of conspecific individuals. In the second case, semivariance was estimated as the mean of the squared difference in mean tree growth for all pairs of individuals with an individual of the focal species and one of another species.  For each species, we compared the semivariances obtained with conspecifics only to the ones obtained with heterospecifics using a Mann-Whitney test. 
...
Results_var_comp %>%
  ...
  kableExtra::kbl(booktabs = TRUE, caption="Comparison of local intra- and interspecific variability in mean annual tree growth for three tropical forest sites. The variability was estimated with the semivariance and the comparison was performed with a Mann-Whitney’s test. The semivariances were computed for all species with > 5 individuals and > 5 heterospecific neighbours within 100 m in the same plot, and considering pairs of individuals that were less than 100 m apart and in the same plot. Shown are the proportion of species, and of corresponding individuals, for which (i) intraspecific variability was significantly lower than interspecific variability, (ii) the difference between inter- and intraspecific variabilities was not significant, or (iii) intraspecific variability was significantly higher than interspecific variability.") %>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position", "scale_down"),
                full_width = F) %>%
  kableExtra::pack_rows("Paracou", start_row = 1, end_row = 2) %>% 
  kableExtra::pack_rows("Uppangala", start_row = 3, end_row = 4)%>%
...
In all three tropical forest sites, growth semivariance was locally significantly higher among heterospecifics than among conspecifics in a large proportion of the tested cases (`r round(as.numeric(Results_var_comp[1,2]))`%, `r round(as.numeric(Results_var_comp[3,2]))`% and `r round(as.numeric(Results_var_comp[5,2]))`% of the species, corresponding to `r round(as.numeric(Results_var_comp[2,2]))`%, `r round(as.numeric(Results_var_comp[4,2]))`% and `r round(as.numeric(Results_var_comp[6,2]))`% of the individuals in the Paracou, Uppangala and BCI sites respectively; Table \@ref(tab:ResultsVarianceComparison)).
...
kableExtra::kbl(Results_moran_undisturbed, booktabs = T, caption = "Spatial autocorrelation of the growth of conspecific individuals in the Paracou site, in plots that were not artificially disturbed. Shown are the proportion of species, and of corresponding individuals, in percent, for which mean tree growth is significantly spatially autocorrelated. The spatial autocorrelation of mean tree growth was tested using Moran’s I index.")%>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position", "scale_down", full_width = F))
```

Considering the undisturbed plots only, the proportion of individuals of species with a significant Moran’s I test was `r round(as.numeric(Results_moran_undisturbed[2,2]))`%, much lower than when including the disturbed plots (`r round(as.numeric(Results_var_comp[2,2]))`%, see Table \@ref(tab:MoransResults); Table \@ref(tab:ResultsMoranParacouUndisturbed)). This corroborates our hypothesis that  the openness of the canopy triggered important spatial structure in disturbed plots due to light gaps, and as in these gaps, trees tend to grow faster, the spatial structure of growth is stronger.
...
Therefore, we computed a competition index $C_{i,t}$ to integrate this effect in a growth model. The competition index is computed for each tree which is not on the edge of a plot. It is the sum of the basal areas (BA) of the 8 direct neighbours (thanks to the plot design, dividing by the area is not needed). $C_{i,t}$ was then log-transformed. Dead neighbours are considered as having a null BA.
...
kableExtra::kbl(
  ...
  escape=F,
  caption = "Residual variance partitioning of tree growth in a clonal *Eucalyptus* plantation. Shown are the mean posterior estimates of the growth model fitted for the clonal *Eucalyptus* plantation as well as the corresponding estimation errors. The individual mean growth was modeled with an intercept, fixed effects for the diameter and the competition level, and with random effects for individuals, sites (experimental blocks), genotypes, and dates (times of the censuses). This allowed to compare the variance associated to each of these random effects, as would be done in a classical variance partitioning, and more particularly to compare the effect of genotype and individual identity."
  ) %>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position", "scale_down"), full_width = F)%>%
...
```{r Chart, fig.cap="Proportions of the components of the unexplained variance."}
Res_var_partition <- data.frame(
...
Importantly, variability between individuals within genotypes (`r Res_var_partition[which(Res_var_partition$category=="Individual"),]$amount`%) is higher than between genotypes (`r Res_var_partition[which(Res_var_partition$category=="Genotype"),]$amount`%). This shows that there is an observed individual variability within genotypes even if trees are clones. Estimated individual variability within genotypes can only be caused by exogenous factors in this particular case. Individual effects can be due to the micro-environment where each tree thrives in, but also to some individual history, such as seedling manipulation and plantation.

The block had the littlest impact with `r Res_var_partition[which(Res_var_partition$category=="Block"),]$amount`% of the variance explained. This is probably due to the fact that environmental conditions between blocks are quite homogeneous.
...
where $1 \leq i \leq 500$ are the individuals; $1 \leq j \leq 2$ are the species; $Y$ is a response variable, for instance growth; $X_1$ to $X_N$ are explanatory variables, for instance environmental variables, that can vary with time $t$; and the $\beta_{k,i}$ , $1 \leq k \leq N$ depict the species-specific responses to these environmental variables. As we consider clones, individuals of the same species respond the same way to environmental variables, and variation in $Y$ among individuals within species is due to differences in the environment where each individual thrives.
...
The difference between the species is imposed by those parameters. Here, species 1 was more competitive on average thanks to its higher intercept ($\beta_0$) and response to the first environmental variable ($\beta_1$).
...
These two virtual datasets (with and without genetic variability) were then analysed with a “Partial knowledge model”, which represents the model derived by ecologists when using datasets obtained from an incomplete characterisation of the environment: only a few variables (here only one, $X_1$) are actually measured and taken into account.
...
We ran this model twice, with the datasets generated with and without genetic IV. These models were fitted in a hierarchical Bayesian framework using the brms package [@burkner_brms_2017 ; @burkner_advanced_2018] with 10,0000 iterations, a warming period of 50,000 iterations and a thinning of 50. We obtained 1,000 estimates per parameter.
...
kableExtra::kbl(
  ...
  escape = F,
  caption = "Statistical analysis of a virtual response variable generated for individual clones of two species thriving in a multidimensional environment. Shown are the mean posterior estimates of the partial knowledge model of the response variable data generated by the perfect knowledge model, as well as the corresponding estimation errors. The logarithm of the response variable was first computed as a linear function of ten simulated environmental variables representing the high dimensionality of the niche (perfect knowledge model). This virtual response variable was then modelled using only one of those environmental variables with an intercept and a slope for each species, and random individual effects (partial knowledge model). Vb, the variance of the individual random effects representing the intraspecific variability, was significantly positive and much larger than V the residual variance."
  ) %>%
...
We inferred a high IV even in the absence of genetic IV (Table \@ref(tab:brmsSummary)). Therefore, observed IV does not necessarily reveal intrinsic (mainly genetic) IV, but can also reveal hidden dimensions of the environment.
We used the model parameters to plot the relationship between $Y$ and $X_1$ accounting for IV.
...
```{r PlotGeneticIV, fig.cap="Virtual response variable generated for individual clones of two species thriving in a multidimensional environment as a function of the only environmental variable that was assumed to be observed. The logarithm of the response variable was first computed as a linear function of ten environmental variables (perfect knowledge model), but is then analysed using a linear function with only one of the ten environmental variables, with a random individual effect (partial knowledge model). The estimated intraspecific variability (IV) was illustrated with envelopes, with (dotted lines) without (plain lines) genetic IV, and the mean posterior of each species with thick lines. The non-genetic IV is then due to the variation in space and time of the nine unobserved environmental variables."}

knitr::include_graphics(here::here("outputs", "theoretical_model", "figures", "Partial_knowledge_model_genetic_var.png"))
```


```{r TwoPanelsFigure, fig.cap="(A) Positions of I=600 individuals from J=2 species in a landscape defined by a square grid of C × C cells (C=500). The background colour indicates the value of the environmental variable X1 (e.g., light) on each cell at date t. The response (e.g., growth) of each individual, which depends on the environment within each cell, is also indicated by a colour scale. (B) Response as a function of the observed environmental variable X1 for the two species. Points represent the data {Y, X1}. The thick lines represent the predictive posterior means for the two species. The envelopes delimited by two thin lines represents the 95 % credible intervals of the predictive posterior marginalized over individuals (taking into account inferred intraspecific variability). The envelopes thus represent the intraspecific variability which is due to the N-1 unobserved environmental variables."}
...
Figure \@ref(fig:TwoPanelsFigure) represents the virtual landscape of $X_1$ and the corresponding individual response and the plot of $Y$ versus $X_1$ (without genetic variability) next to each other, helping to visualise this virtual experiment.

In the model without genetic IV, the unobserved variation in the environment resulted in an important inferred IV. The performances of the two species can intersect, which means that the competitive hierarchy among the two species can be locally reversed depending on the micro-environmental variation, offering opportunities for the coexistence of the two species in a variable environment. 
...
caption = "Spatial autocorrelation in a virtual response variable generated for individual clones of two species thriving in a multidimensional environment. Shown are the values and significance of Moran’s I index computed for the response variable, for each species separately and for all individuals together. Moran’s indices were positive and significant, indicating a positive spatial autocorrelation of the response variable in all cases.") %>%
...
Finally, we used semivariograms to visualise the spatial autocorrelation of the response variable and to test whether the response variable was more similar within conspecifics than within heterospecifics.
...
```{r SemivariogramFigure, warning=FALSE, fig.cap="Spatial autocorrelation of a virtual response variable generated for individual clones of two species thriving in a multidimensional environment. This semivariogram represents the semivariance of the individual mean response variable as a function of the distance between individuals. The semivariance increased rapidly with the distance, showing that the response variable is strongly spatially autocorrelated in general and within species. The semivariance of all individuals taken together (purple curve) is higher than the semivariance of conspecifics (red and blue curves), which means that intraspecific variability is lower than interspecific variability."}
...
# Code used to generate Figure 1 in the main text: *Reinterpreting observed intraspecific variability (IV): from niche widening to niche projection into a high-dimensional environment*.
