...
Figure \@ref(fig:IncludePlotUppangala) shows the location of the trees in Uppangala.
...
Figure \@ref(fig:IncludePlotBCI) shows the location of the selected trees in BCI.
...
We used the plots 1 to 15 of the Paracou site.
...
We computed annualized growth between two censuses and removed all trees of undetermined species. Growth was computed as the difference of DBH (Diameter at Breast Height) between two consecutive censuses, divided by the time period between those two censuses.
...
We then removed estimates with  a growth < -2 mm/an or > 100 mm/an and removed the years before the perturbations were performed and the biodiversity plots were added (1985-1991).
...
This led to a dataset with `r nrow(Paracou_no_mean)` growth estimates for `r length(unique(Paracou_no_mean$idTree))` trees of `r length(unique(Paracou_no_mean$Sp))` species.

We finally computed the mean annual growth for each individual tree in this final dataset as the difference of DBH between the first and the last time a tree was measured, divided by the period between those two measurements.
...
We removed the census dates which were not common for all plots.
...
We then computed annualized growth between two censuses as for the Paracou site and removed estimates with a growth >100 mm/y or < -2 mm/y.
...
This led to a dataset with `r nrow(Uppangala_no_mean)` growth estimates for `r length(unique(Uppangala_no_mean$TreeID))` trees of `r length(unique(Uppangala_no_mean$SpCode))` species.


We finally computed mean annual growth as for the Paracou site.
...
We removed the trees with missing DBH measures and trees with DBH < 10 cm for consistency with the Paracou and Uppangala sites.
...
We then computed annualized growth as for the Paracou site and removed estimates with a growth >100 mm/y or < -2 mm/y.
...
This led to a dataset with `r nrow(BCI_no_mean)` growth estimates for `r length(unique(BCI_no_mean$treeID))` trees of `r length(unique(BCI_no_mean$sp))` species.
...
These diagrams (Figure \@ref(fig:AbundanceDiagrams)) enable us to visualise how diverse the tropical forest is, but also that there are few dominant species and many rare species (long threshold).
