...
```{r Charge the data, include=FALSE}
Full_Data <- list()
for (k in 1:6) {
  l = c(3, 4, 6, 8, 12, 14)[k]
...
```{r Plots_growth_light, echo=FALSE, fig.show="hold", out.width="33%"}
...
```{r Competition index, eval=FALSE, include=FALSE}
...
g <-brms::mcmc_plot(brms_mod, type="trace")+
  ggplot2::labs(title="Trace of the posteriors")
...
g <- brms::mcmc_plot(brms_mod, type="dens")+
  ggplot2::labs(title="Density of the posteriors")
...
g <- bayesplot::mcmc_trace(Ranef_date)+
  ggplot2::labs(title="Temporal random effects")
...
g <- bayesplot::mcmc_trace(Ranef_gen)+
  ggplot2::labs(title="Genotype random effects")
...
g <- bayesplot::mcmc_trace(Ranef_site)+
  ggplot2::labs(title="Site random effects")
...
```{r Plots_ranef, echo=FALSE, fig.show="hold", out.width="33%"}
