---
title: "Multiple insights on the nature of IV and its consequences on individual and species differences"
output:
...
We used literature and data analyses of various nature to support our arguments. Figure \@ref(fig:MultipleInsights) illustrates how these elements articulate together.

```{r MultipleInsights, echo=FALSE, fig.cap="The multiple of insights used in the paper to argument towards the rethinking the nature of intraspecific variability and its consequences on species coexistence"}
knitr::include_graphics(here::here("data", "Multiple_insights.png"))
```
...
```{r LoadParacouNoMean2, include=FALSE}
load(here::here("outputs", "tropical_analysis", "Paracou_no_mean.RData"))
```
...
To quantify the relative importance of intraspecific and interspecific variability for each site, we built a model including a species random effect and an individual random effect on the intercept. In order to compare to classic growth models, and estimate an intraspecific variability within diameter classes, we also added an intercept and a diameter fixed effect.
...
$b_{0j} \sim \mathcal{N}(mean=0, var=V_bj), iid$

$d_{0j} \sim \mathcal{N}(mean=0, var=V_bi), iid$

$\epsilon_{ijt} \sim  \mathcal{N}(mean=0, var=V), iid$

Hyperpriors

$V_{bj} \sim t(3, 0, 2.5), iid$

$V_{bi} \sim t(3, 0, 2.5), iid$

$V \sim t(3, 0, 2.5), iid$

Where $i$ is the individual and $j$ the species.
The model included two fixed effects ($\beta_0$ and $\beta_1$), a random species effect $b_{0j}$ on the intercept and a random individual effect $d_{0i}$ on the intercept. $V_bi$ is the intraspecific variance and $V_bj$ is the interspecific variance.

The following models were fitted in a hierarchical Bayesian framework using the brms package [@burkner_brms_2017 ; @burkner_advanced_2018 ] with 10,000 iterations, a warming period of 5,000 iterations and a thinning of 1/5, and 4 MCMC chains with different initial values. We obtained 1,000 estimates per parameter per MCMC chain.

The data was scaled before inference.
...
brms_mod <- brms::brm(formula = logG ~ 1 + logD + (1|IdentSpecies) + (1|IdentTree), 
               data = Data_stan_Paracou,
               prior= prior_brms,
               iter = 10000 , warmup = 5000, thin = 5,
               chains = 4, cores=4)


save(brms_mod, file = here::here("outputs", "tropical_analysis", "brms_mod_Paracou_revision.RData"))

```

```{r LoadBRMSParacou, include=FALSE}
load(here::here("outputs", "tropical_analysis", "brms_mod_Paracou_revision.RData"))
```
...
save(brms_mod, file=here::here("outputs", "tropical_analysis", "brms_mod_Uppangala_revision.RData"))

```

```{r LoadModelUppangala, include=FALSE}
load(here::here("outputs", "tropical_analysis", "brms_mod_Uppangala_revision.RData"))
```
...
save(brms_mod, file = here::here("outputs", "tropical_analysis", "brms_mod_BCI_revision.RData"))

```

```{r LoadModelBCI, include=FALSE}
load(here::here("outputs", "tropical_analysis", "brms_mod_BCI_revision.RData"))
```
...
kableExtra::kbl(
  ...
  booktabs = T,
  escape = F,
  col.names = c(" ", "Intercept ($\\beta_0$)", "Diameter ($\\beta_1$)", "Species variance ($V_bj$)", "Individual variance ($V_bi$)", "Residual variance ($V$)"),
...
```{r SummaryModelsComplete}
Res_mods <- 
  data.frame(
    Amount = rep(c("Estimate", "Estimation error", "95\\% interval", "R-hat", "Bulk ESS", "Tail ESS"), 3),
    Intercept = c(
      format(as.numeric(Sum_paracou$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_paracou$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_paracou$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_paracou$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$fixed[1,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_uppangala$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_uppangala$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_uppangala$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[1,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_BCI$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_BCI$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_BCI$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[1,7]), digits = 2, scientific = T)
      ),
    Diameter = c(
      format(as.numeric(Sum_paracou$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_paracou$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_paracou$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_paracou$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$fixed[2,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_uppangala$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_uppangala$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_uppangala$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$fixed[2,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_BCI$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_BCI$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_BCI$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$fixed[2,7]), digits = 2, scientific = T)
      ),
    Vbj = c(
      format(as.numeric(Sum_paracou$random$IdentSpecies[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$random$IdentSpecies[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_paracou$random$IdentSpecies[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_paracou$random$IdentSpecies[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_paracou$random$IdentSpecies[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$random$IdentSpecies[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$random$IdentSpecies[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentSpecies[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentSpecies[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_uppangala$random$IdentSpecies[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_uppangala$random$IdentSpecies[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_uppangala$random$IdentSpecies[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentSpecies[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentSpecies[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentSpecies[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentSpecies[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_BCI$random$IdentSpecies[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_BCI$random$IdentSpecies[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_BCI$random$IdentSpecies[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentSpecies[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentSpecies[7]), digits = 2, scientific = T)
      ),
    Vbi = c(
      format(as.numeric(Sum_paracou$random$IdentTree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$random$IdentTree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_paracou$random$IdentTree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_paracou$random$IdentTree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_paracou$random$IdentTree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$random$IdentTree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$random$IdentTree[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentTree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentTree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_uppangala$random$IdentTree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_uppangala$random$IdentTree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_uppangala$random$IdentTree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentTree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$random$IdentTree[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentTree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentTree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_BCI$random$IdentTree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_BCI$random$IdentTree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_BCI$random$IdentTree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentTree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$random$IdentTree[7]), digits = 2, scientific = T)
      ),
    V=c(
      format(as.numeric(Sum_paracou$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_paracou$spec_pars[,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_paracou$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_paracou$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_paracou$spec_pars[,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_uppangala$spec_pars[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_uppangala$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_uppangala$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_uppangala$spec_pars[,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_BCI$spec_pars[,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_BCI$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_BCI$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_BCI$spec_pars[,7]), digits = 2, scientific = T)
      )
    )
kableExtra::kbl(
  Res_mods,
  booktabs = T,
  escape = F,
  col.names = c(" ", "Intercept ($\\beta_0$)", "Diameter ($\\beta_1$)", "Species variance ($V_{bj}$)", "Individual variance ($V_{bi}$)", "Residual variance ($V$)"),
  caption = "Summary of the model's outputs"
  ) %>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position", "scale_down"), full_width = F)%>%
  kableExtra::pack_rows("Paracou", start_row = 1, end_row = 6) %>% 
  kableExtra::pack_rows("Uppangala", start_row = 7, end_row = 12)%>%
  kableExtra::pack_rows("BCI", start_row = 13, end_row = 18)
```
...
We then computed the proportion of species with a significantly and positively spatially autocorrelated growth (with a 0.05 alpha risk) and the proportion of species with non significant spatial autocorrelation, as well as the corresponding proportion of individuals. The proportions are computed relative to the species on which the test was performed.
...
load(here::here("outputs", "tropical_analysis", "Moran_I_Paracou.RData"))
load(here::here("outputs", "tropical_analysis", "Moran_I_Uppangala.RData"))
load(here::here("outputs", "tropical_analysis", "Moran_I_BCI.RData"))

Res_signif_Moran <- data.frame(R_2 = numeric(3),
                               P_val = numeric(3))
rownames(Res_signif_Moran) <- c("Paracou", "Uppangala", "BCI")

model_signif_Moran_Paracou <- glm(factor(signif) ~ nb_ind,family=binomial(link='logit'),data=na.omit(Moran_I_Paracou))

model_signif_Moran_Uppangala <- glm(factor(signif) ~ nb_ind,family=binomial(link='logit'),data=na.omit(Moran_I_Uppangala))

model_signif_Moran_BCI <- glm(factor(signif) ~ nb_ind,family=binomial(link='logit'),data=na.omit(Moran_I_BCI))

#McFadden’s R-squared value
Res_signif_Moran[1,1] <- with(summary(model_signif_Moran_Paracou), 1 - deviance/null.deviance)
Res_signif_Moran[2,1] <- with(summary(model_signif_Moran_Uppangala), 1 - deviance/null.deviance)
Res_signif_Moran[3,1] <- with(summary(model_signif_Moran_BCI), 1 - deviance/null.deviance)

#P-value of the number of individuals
Res_signif_Moran[1,2] <- summary(model_signif_Moran_Paracou)$coef[2,4]
Res_signif_Moran[2,2] <- summary(model_signif_Moran_Uppangala)$coef[2,4]
Res_signif_Moran[3,2] <- summary(model_signif_Moran_BCI)$coef[2,4]

save(Res_signif_Moran, file=here::here("outputs", "tropical_analysis", "Signif_abund_Moran.RData"))

Moran_I_Paracou$signif <- as.factor(Moran_I_Paracou$signif)
Moran_I_Uppangala$signif <- as.factor(Moran_I_Uppangala$signif)
Moran_I_BCI$signif <- as.factor(Moran_I_BCI$signif)

mu <- plyr::ddply(Moran_I_Paracou[which(Moran_I_Paracou$signif!="NaN"),], "signif", plyr::summarise, grp.mean=mean(nb_ind))
A <- ggplot2::ggplot(Moran_I_Paracou[which(Moran_I_Paracou$signif!="NaN"),],
                     ggplot2::aes(x=nb_ind, color=signif, fill=signif))+
  ggplot2::geom_histogram(position="identity", alpha=0.5)+
  ggplot2::labs(title="Paracou", x="Number of individuals", y="Frequency")+
  ggplot2::geom_vline(data=mu, ggplot2::aes(xintercept=grp.mean, linetype=signif),
             color="black")+
  ggplot2::scale_color_brewer(palette="Paired")+
  ggplot2::scale_fill_brewer(palette="Paired")+
  ggplot2::theme(text=ggplot2::element_text(size=20))

mu <- plyr::ddply(Moran_I_Uppangala[which(Moran_I_Uppangala$signif!="NaN"),], "signif", plyr::summarise, grp.mean=mean(nb_ind))
B <- ggplot2::ggplot(Moran_I_Uppangala[which(Moran_I_Uppangala$signif!="NaN"),],
                     ggplot2::aes(x=nb_ind, color=signif, fill=signif))+
  ggplot2::geom_histogram(position="identity", alpha=0.5)+
  ggplot2::labs(title="Uppangala", x=ggplot2::element_blank(), y=ggplot2::element_blank())+
  ggplot2::geom_vline(data=mu, ggplot2::aes(xintercept=grp.mean, linetype=signif),
             color="black")+
  ggplot2::scale_color_brewer(palette="Paired")+
  ggplot2::scale_fill_brewer(palette="Paired")+
...
mu <- plyr::ddply(Moran_I_BCI[which(Moran_I_BCI$signif!="NaN"),], "signif", plyr::summarise, grp.mean=mean(nb_ind))
C <- ggplot2::ggplot(Moran_I_BCI[which(Moran_I_BCI$signif!="NaN"),],
                     ggplot2::aes(x=nb_ind, color=signif, fill=signif))+
  ggplot2::geom_histogram(position="identity", alpha=0.5)+
  ggplot2::labs(title="BCI", x=ggplot2::element_blank(), y=ggplot2::element_blank())+
  ggplot2::geom_vline(data=mu, ggplot2::aes(xintercept=grp.mean, linetype=signif),
             color="black")+
  ggplot2::scale_color_brewer(palette="Paired")+
  ggplot2::scale_fill_brewer(palette="Paired")+
...
Plot_Moran_signif_abund <- ggpubr::ggarrange(A, B, C, nrow = 1, ncol = 3, common.legend = TRUE, legend="bottom")
ggplot2::ggsave(filename = "Moran_signif_abund.png", plot = Plot_Moran_signif_abund, path=here::here("outputs", "tropical_analysis", "figures"), device = "png", width = 15, height = 6, bg="white")
...
for (plot in sort(unique(Data_Paracou$Plot))) {
  ...
  for (file in 0:(nfiles-1)){
    file.rename(here::here("analyses", glue::glue("file{file}_{plot}_P.txt")), here::here("outputs", "tropical_analysis", "Dists_Paracou", glue::glue("file{file}_{plot}_P.txt")))
  }
}
...
for (plot in sort(unique(Data_Uppangala$Plot))){
  ...
  for (file in 0:(nfiles-1)){
    file.rename(here::here("analyses", glue::glue("file{file}_{plot}_U.txt")), here::here("outputs", "tropical_analysis", "Dists_Uppangala", glue::glue("file{file}_{plot}_U.txt")))
  }
}
...
for (file in 0:(nfiles-1)){
  file.rename(here::here("analyses", glue::glue("file{file}_1_B.txt")), here::here("outputs", "tropical_analysis", "Dists_BCI", glue::glue("file{file}_1_B.txt")))
}
...
```{r SignificanceAbundanceVarComp, message=FALSE, warning=FALSE, fig.cap=paste0("Histograms of the abundances of species which present a significant (dark blue) or insignificant (light blue) Moran’s I test (upper panels) and semivariance comparison test (bottom panels). The dotted lines represent the mean abundance across species with significant (plain) and insignificant (dashed) test in each case. Using a simple generalized linear model with a logit link function, we also showed that the abundance of species is a significant predictor of test significance in both cases (McFadden’s R-squared value = ", format(Res_signif_Moran[1,1], digits=2), " for Moran’s I and ", format(Res_signif_Var_comp[1,1], digits=2), " for semivariance comparison for Paracou, ", format(Res_signif_Moran[2,1], digits=2), " and ", format(Res_signif_Var_comp[2,1], digits=2), " for Uppangala, ", format(Res_signif_Moran[3,1], digits=2), " and ", format(Res_signif_Var_comp[3,1], digits=2), " for BCI; p-value of the effect of abundance on significance = ", format(Res_signif_Moran[1,2], digits=2), " and ", format(Res_signif_Var_comp[1,2], digits=2), " for Paracou, ", format(Res_signif_Moran[2,2], digits=2), " and ", format(Res_signif_Var_comp[2,2], digits=2), " for Uppangala, ", format(Res_signif_Moran[3,2], digits=2), "and ", format(Res_signif_Var_comp[3,2], digits=2), " for BCI).")}
Var_comp_Paracou$Signif <- as.factor(Var_comp_Paracou$Signif)
Var_comp_Uppangala$Signif <- as.factor(Var_comp_Uppangala$Signif)
Var_comp_BCI$Signif <- as.factor(Var_comp_BCI$Signif)

## Test to see if the number of individuals explains the significance ##

Res_signif_Var_comp <- data.frame(R_2 = numeric(3),
                                  P_val = numeric(3))
rownames(Res_signif_Var_comp) <- c("Paracou", "Uppangala", "BCI")

model_signif_Var_comp_Paracou <- glm(factor(Signif) ~ Nb_ind,family=binomial(link='logit'),data=na.omit(Var_comp_Paracou))

model_signif_Var_comp_Uppangala <- glm(factor(Signif) ~ Nb_ind,family=binomial(link='logit'),data=na.omit(Var_comp_Uppangala))

model_signif_Var_comp_BCI <- glm(factor(Signif) ~ Nb_ind,family=binomial(link='logit'),data=na.omit(Var_comp_BCI))

#McFadden’s R-squared value
Res_signif_Var_comp[1,1] <- with(summary(model_signif_Var_comp_Paracou), 1 - deviance/null.deviance)
Res_signif_Var_comp[2,1] <- with(summary(model_signif_Var_comp_Uppangala), 1 - deviance/null.deviance)
Res_signif_Var_comp[3,1] <- with(summary(model_signif_Var_comp_BCI), 1 - deviance/null.deviance)

#P-value of the number of individuals
Res_signif_Var_comp[1,2] <- summary(model_signif_Var_comp_Paracou)$coef[2,4]
Res_signif_Var_comp[2,2] <- summary(model_signif_Var_comp_Uppangala)$coef[2,4]
Res_signif_Var_comp[3,2] <- summary(model_signif_Var_comp_BCI)$coef[2,4]

save(Res_signif_Var_comp, file=here::here("outputs", "tropical_analysis", "Signif_abund_Var_comp.RData"))

mu <- plyr::ddply(Var_comp_Paracou[which(Var_comp_Paracou$Signif!="NA"),], "Signif", plyr::summarise, grp.mean=mean(Nb_ind))
A <- ggplot2::ggplot(Var_comp_Paracou[which(Var_comp_Paracou$Signif!="NA"),],
                     ggplot2::aes(x=Nb_ind, color=Signif, fill=Signif))+
  ggplot2::geom_histogram(position="identity", alpha=0.5)+
  ggplot2::labs(title="Paracou", x="Number of individuals", y="Frequency")+
  ggplot2::geom_vline(data=mu, ggplot2::aes(xintercept=grp.mean, linetype=Signif),
...
mu <- plyr::ddply(Var_comp_Uppangala[which(Var_comp_Uppangala$Signif!="NA"),], "Signif", plyr::summarise, grp.mean=mean(Nb_ind))
B <- ggplot2::ggplot(Var_comp_Uppangala[which(Var_comp_Uppangala$Signif!="NA"),],
                     ggplot2::aes(x=Nb_ind, color=Signif, fill=Signif))+
  ggplot2::geom_histogram(position="identity", alpha=0.5)+
  ggplot2::labs(title="Uppangala", x=ggplot2::element_blank(), y=ggplot2::element_blank())+
  ggplot2::geom_vline(data=mu, ggplot2::aes(xintercept=grp.mean, linetype=Signif),
...
mu <- plyr::ddply(Var_comp_BCI[which(Var_comp_BCI$Signif!="NA"),], "Signif", plyr::summarise, grp.mean=mean(Nb_ind))
C <- ggplot2::ggplot(Var_comp_BCI[which(Var_comp_BCI$Signif!="NA"),],
                     ggplot2::aes(x=Nb_ind, color=Signif, fill=Signif))+
  ggplot2::geom_histogram(position="identity", alpha=0.5)+
  ggplot2::labs(title="BCI", x=ggplot2::element_blank(), y=ggplot2::element_blank())+
  ggplot2::geom_vline(data=mu, ggplot2::aes(xintercept=grp.mean, linetype=Signif),
...
Plot_Var_comp_signif_abund <- ggpubr::ggarrange(A, B, C, nrow = 1, ncol = 3, common.legend = TRUE, legend="bottom")
ggplot2::ggsave(filename = "Var_comp_signif_abund.png", plot = Plot_Var_comp_signif_abund, path=here::here("outputs", "tropical_analysis", "figures"), device = "png", width = 15, height = 6, bg="white")
knitr::include_graphics(here::here("outputs", "tropical_analysis", "figures", "Var_comp_signif_abund.png"))

Plot_signif_abund <- ggpubr::ggarrange(Plot_Moran_signif_abund, Plot_Var_comp_signif_abund, nrow=2, ncol=1, labels=c("A", "B"))
ggplot2::ggsave(filename = "Plot_signif_abund.png", plot = Plot_signif_abund, path=here::here("outputs", "tropical_analysis", "figures"), device = "png", width = 15, height = 10, bg="white")
...
```{r ParacouUndisturbed, eval=FALSE, include=FALSE}
...
param2 = c(0.2, 0.1, abs(runif(5, -0.05, 0.05)), -abs(runif(4, -0.05, 0.05)))
...
```{r SpatialModels, eval=FALSE, include=FALSE}
#Matrixes of variables
  ...
  for(k in 1:10){
    mods[[k]] <- gstat::gstat(formula=z~1, locations=~x+y, dummy=T, beta=0, model=gstat::vgm(psill=1,model="Sph",range=50, nugget=0), nmax=20)
  }
  
```

```{r EnvironmentalVariables, eval=FALSE, include=FALSE}
#this chunk takes time and can be skipped if already ran once (files will be loaded)
...
For the first environmental variable which had a stronger impact on $Y$ values and another randomly drawn environmental variable, we computed values at $t_1$ as $t_0 + \epsilon$, $\epsilon \sim \mathcal{N}(0, 1)$ (they randomly increase or decrease).
For two other environmental variables randomly drawn, $X_{t_1} = X_{t_0} + |\epsilon|, \epsilon \sim \mathcal{N}(0, 1)$ (they increase) and for two other environmental variables, $X_{t_1} = X_{t_0} - |\epsilon|, \epsilon \sim \mathcal{N}(0, 1)$ (they decrease).
...
Vars_t1[,3] <- Vars_t1[,3] + abs(rnorm(1, mean=0, sd=1))
Vars_t1[,2+changing_variables[1]] <- Vars_t1[,2+changing_variables[1]] + abs(rnorm(1, mean=0, sd=1))
Vars_t1[,2+changing_variables[2]] <- Vars_t1[,2+changing_variables[2]] + abs(rnorm(1, mean=0, sd=1))
Vars_t1[,2+changing_variables[3]] <- Vars_t1[,2+changing_variables[3]] - abs(rnorm(1, mean=0, sd=1))
Vars_t1[,2+changing_variables[4]] <- Vars_t1[,2+changing_variables[4]] - abs(rnorm(1, mean=0, sd=1))
Vars_t1[,2+changing_variables[5]] <- Vars_t1[,2+changing_variables[5]] + rnorm(1, mean=0, sd=1)
...
```{r brmsSummaryComplete}
Res_mods <- 
  data.frame(
    rep(c("Estimate", "Estimation error", "95\\% interval", "R-hat", "Bulk ESS", "Tail ESS"), 4),
    c(
      format(as.numeric(Sum_sp1$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$fixed[1,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[1,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1_IV$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1_IV$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1_IV$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[1,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[1,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[1,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1_IV$fixed[1,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2_IV$fixed[1,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2_IV$fixed[1,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[1,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[1,7]), digits = 2, scientific = T)
      ),
    c(
      format(as.numeric(Sum_sp1$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$fixed[2,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$fixed[2,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1_IV$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1_IV$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1_IV$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$fixed[2,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[2,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[2,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2_IV$fixed[2,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2_IV$fixed[2,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2_IV$fixed[2,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[2,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$fixed[2,7]), digits = 2, scientific = T)
      ),
    c(
      format(as.numeric(Sum_sp1$random$tree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$random$tree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1$random$tree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1$random$tree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1$random$tree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$random$tree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$random$tree[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$random$tree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$random$tree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2$random$tree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2$random$tree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2$random$tree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$random$tree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$random$tree[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$random$tree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$random$tree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1_IV$random$tree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1_IV$random$tree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1_IV$random$tree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$random$tree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$random$tree[7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$random$tree[1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$random$tree[2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2_IV$random$tree[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2_IV$random$tree[4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2_IV$random$tree[5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$random$tree[6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$random$tree[7]), digits = 2, scientific = T)
      ),
    c(
      format(as.numeric(Sum_sp1$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1$spec_pars[,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1$spec_pars[,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2$spec_pars[3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2$spec_pars[,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp1_IV$spec_pars[,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp1_IV$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp1_IV$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp1_IV$spec_pars[,7]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$spec_pars[,1]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$spec_pars[,2]), digits = 2, scientific = T),
      paste(format(as.numeric(Sum_sp2_IV$spec_pars[,3]), digits = 2, scientific = T), "-", format(as.numeric(Sum_sp2_IV$spec_pars[,4]), digits = 2, scientific = T)),
      format(as.numeric(Sum_sp2_IV$spec_pars[,5]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$spec_pars[,6]), digits = 2, scientific = T),
      format(as.numeric(Sum_sp2_IV$spec_pars[,7]), digits = 2, scientific = T)
      )
    )
kableExtra::kbl(
  Res_mods,
  ...
  kableExtra::pack_rows("Species 1 (no random IV)", start_row = 1, end_row = 6) %>% 
  kableExtra::pack_rows("Species 2 (no random IV)", start_row = 7, end_row = 12)%>%
  kableExtra::pack_rows("Species 1 (with random IV)", start_row = 13, end_row = 18) %>% 
  kableExtra::pack_rows("Species 2 (with random IV)", start_row = 19, end_row = 24)%>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position"), full_width = F)
```
...
```{r GrowthNoIntrinsicIV}
 ...
 #Create a sequence of 100 values of X
    ...
    Sd_b_sp1 <- mean(c(MCMC_brms_sp_1[[1]][,3], MCMC_brms_sp_1[[2]][,3], MCMC_brms_sp_1[[3]][,3], MCMC_brms_sp_1[[4]][,3]))
    Sd_b_sp1_IV <- mean(c(MCMC_brms_sp_1_IV[[1]][,3], MCMC_brms_sp_1_IV[[2]][,3], MCMC_brms_sp_1_IV[[3]][,3], MCMC_brms_sp_1_IV[[4]][,3]))
    Sd_b_sp2 <- mean(c(MCMC_brms_sp_2[[1]][,3], MCMC_brms_sp_2[[2]][,3], MCMC_brms_sp_2[[3]][,3], MCMC_brms_sp_2[[4]][,3]))
    Sd_b_sp2_IV <- mean(c(MCMC_brms_sp_2_IV[[1]][,3], MCMC_brms_sp_2_IV[[2]][,3], MCMC_brms_sp_2_IV[[3]][,3], MCMC_brms_sp_2_IV[[4]][,3]))
    
    #Without intrinsic variation#
    Param_sp1 <- data.frame(beta1_sp1=numeric(4000), beta2_sp1=numeric(4000), ind=numeric(4000))
    Param_sp1[,1] <- c(MCMC_brms_sp_1[[1]][,1], MCMC_brms_sp_1[[2]][,1], MCMC_brms_sp_1[[3]][,1], MCMC_brms_sp_1[[4]][,1])
    Param_sp1[,2] <- c(MCMC_brms_sp_1[[1]][,2], MCMC_brms_sp_1[[2]][,2], MCMC_brms_sp_1[[3]][,2], MCMC_brms_sp_1[[4]][,2])
    Param_sp1[,3] <- rnorm(4000, 0, Sd_b_sp1)
    Log_growth_pred_sp1 <- matrix(nrow=4000, ncol=npred)
    for (n in 1:npred) {
      Log_growth_pred_sp1[,n] <- Param_sp1$beta1_sp1 + Param_sp1$beta2_sp1*logLight_seq[n] + Param_sp1$ind
    }
    
    Param_sp2 <- data.frame(beta1_sp2=numeric(4000), beta2_sp2=numeric(4000), ind=numeric(4000))
    Param_sp2[,1] <- c(MCMC_brms_sp_2[[1]][,1], MCMC_brms_sp_2[[2]][,1], MCMC_brms_sp_2[[3]][,1], MCMC_brms_sp_2[[4]][,1])
    Param_sp2[,2] <- c(MCMC_brms_sp_2[[1]][,2], MCMC_brms_sp_2[[2]][,2], MCMC_brms_sp_2[[3]][,2], MCMC_brms_sp_2[[4]][,2])
    Param_sp2[,3] <- rnorm(4000, 0, Sd_b_sp2)
    Log_growth_pred_sp2 <- matrix(nrow=4000, ncol=npred)
    for (n in 1:npred) {
...
#With intrinsic variation#
    ...
    Param_sp1_IV <- data.frame(beta1_sp1_IV=numeric(4000), beta2_sp1_IV=numeric(4000), ind=numeric(4000))
    Param_sp1_IV[,1] <- c(MCMC_brms_sp_1_IV[[1]][,1], MCMC_brms_sp_1_IV[[2]][,1], MCMC_brms_sp_1_IV[[3]][,1], MCMC_brms_sp_1_IV[[4]][,1])
    Param_sp1_IV[,2] <- c(MCMC_brms_sp_1_IV[[1]][,2], MCMC_brms_sp_1_IV[[2]][,2], MCMC_brms_sp_1_IV[[3]][,2], MCMC_brms_sp_1_IV[[4]][,2])
    Param_sp1_IV[,3] <- rnorm(4000, 0, Sd_b_sp1_IV)
    
    Log_growth_pred_sp1_IV <- matrix(nrow=4000, ncol=npred)
    ...
    Param_sp2_IV <- data.frame(beta1_sp2_IV=numeric(4000), beta2_sp2_IV=numeric(4000), ind=numeric(4000))
    Param_sp2_IV[,1] <- c(MCMC_brms_sp_2_IV[[1]][,1], MCMC_brms_sp_2_IV[[2]][,1], MCMC_brms_sp_2_IV[[3]][,1], MCMC_brms_sp_2_IV[[4]][,1])
    Param_sp2_IV[,2] <- c(MCMC_brms_sp_2_IV[[1]][,2], MCMC_brms_sp_2_IV[[2]][,2], MCMC_brms_sp_2_IV[[3]][,2], MCMC_brms_sp_2_IV[[4]][,2])
    Param_sp2_IV[,3] <- rnorm(4000, 0, Sd_b_sp2_IV)
    
    Log_growth_pred_sp2_IV <- matrix(nrow=4000, ncol=npred)
...
As the semivariance between individuals of both species was higher than the semivariance between conspecifics (Figure \@ref(fig:SemivariogramFigure)), the individual response variable was more similar between conspecifics than heterospecifics.


# Code used to generate Figure 3 in the main text: _Reinterpreting observed intraspecific variability (IV): from niche widening to niche projection into a high-dimensional environment_.
...
```{r CodeFigConcept, echo=TRUE, message=FALSE, warning=FALSE}
library(ggplot2)
library(mvtnorm)
library(dplyr)
library(ggpubr)
...
data.grid <- expand.grid(X1 = seq(minX1,maxX1, length.out=200),
                         X2 = seq(minX2, maxX2, length.out=200))

#Species performance
muA = c(0, 0)
muB = c(3, 20)
SigmaA = matrix (c(5, 0, 0, 5), 2, 2) #x and y axes are independent (0)
SigmaB = matrix (c(10, 0, 0, 10), 2, 2)
pA <- data.frame(X1=data.grid$X1,
                 X2=data.grid$X2,
                 Perf=mvtnorm::dmvnorm(x=data.grid,
                                       mean = muA,
                                       sigma = SigmaA))
pB <- data.frame(X1=data.grid$X1,
                 X2=data.grid$X2,
                 Perf=mvtnorm::dmvnorm(x=data.grid,
                                       mean = muB,
                                       sigma = SigmaB))
...
pA_E0 = data.frame(X1=rep(X1_0, 200),
                   X2=seq(minX2, maxX2, length.out=200),
                   Perf=mvtnorm::dmvnorm(x=cbind(
                     rep(X1_0, 200),
                     seq(minX2, maxX2, length.out=200)),
                     mean = muA,
                     sigma = SigmaA))

mean_pA_E0 = mean(pA_E0$Perf)

pB_E0 = data.frame(X1=rep(X1_0, 200),
                   X2=seq(minX2, maxX2, length.out=200),
...
mean_pB_E0 = mean(pB_E0$Perf)

X1_1 = 3

pA_E1 = data.frame(X1=rep(X1_1, 200),
                   X2=seq(minX2, maxX2, length.out=200),
                   Perf=mvtnorm::dmvnorm(
                     x=cbind(
                       rep(X1_1, 200),
...
mean_pA_E1 = mean(pA_E1$Perf)

pB_E1 = data.frame(X1=rep(X1_1, 200),
                   X2=seq(minX2, maxX2, length.out=200),
                   Perf=mvtnorm::dmvnorm(
                     x=cbind(
                       rep(X1_1, 200),
...
plot_A <- ggplot2::ggplot(data=data.frame(Sp=as.factor(c("A","B")),
                                     Mean=c(mean_pA_E0, mean_pB_E0),
                                     Sd=c(sd(pA_E0$Perf), sd(pB_E0$Perf))),
                     aes(x=Sp, y=Mean))+
  ...
  ggplot2::geom_segment(ggplot2::aes(x = 2,
                        ...
                        linetype="dashed",
                        size=0.1)+
  annotate("text", label="pA", x = 0.30, y = mean_pA_E0, size=3)+
  annotate("text", label="pB", x = 0.30, y = mean_pB_E0, size=3)+
  annotate("text", label="Species A", x = 1, y = -0.0053, size=3)+
  annotate("text", label="Species B", x = 2, y = -0.0053, size=3)

plot_C <- ggplot2::ggplot(data=data.frame(Sp=as.factor(c("A","B")),
                                     Mean=c(mean_pA_E0, mean_pB_E0),
                                     Sd=c(sd(pA_E0$Perf), sd(pB_E0$Perf))),
...
plot_D <- ggplot2::ggplot(data=marginals_pA, aes(x=X1, y=mean))+
  ggplot2::geom_line(colour="midnightblue")+
  ggplot2::geom_line(data=marginals_pB, colour="orange")+
  ggplot2::coord_cartesian(xlim=c(minX1, maxX1), ylim = c(-0.004, 0.009), clip = "off")+
  ggplot2::geom_point(ggplot2::aes(x=X1_0, y=mean_pA_E0), colour="midnightblue") +
  ggplot2::geom_point(ggplot2::aes(x=X1_0, y=mean_pB_E0), colour="orange")+
  ggplot2::geom_errorbar(ggplot2::aes(x=X1_0,
                                      ymin=mean_pA_E0-sd(pA_E0$Perf),
                                      ymax=mean_pA_E0+sd(pA_E0$Perf)),
                         colour="midnightblue",
                         width=0.5,
                         size=0.2)+
  ggplot2::geom_errorbar(ggplot2::aes(x=X1_0,
                                      ymin=mean_pB_E0-sd(pB_E0$Perf),
                                      ymax=mean_pB_E0+sd(pB_E0$Perf)),
                         colour="orange",
                         width=0.5,
                         size=0.2)+
  ggplot2::theme(plot.background = ggplot2::element_blank(),
                 ...
                 axis.title.x = element_text(vjust=-3),
                 axis.title.y = element_text(vjust=4))+
  ggplot2::labs(x="X1", y = "Performance")+
  ggplot2::labs(x="X1", y = "Performance")+
  ggplot2::geom_segment(ggplot2::aes(x = X1_0,
                                     y = -0.0045,
                                     xend = X1_0,
                                     yend = max(mean_pA_E0,mean_pB_E0)),
                        linetype="dashed",
                        size=0.1)+
  ggplot2::geom_segment(ggplot2::aes(x = X1_1,
                                     y = -0.0045,
                                     xend = X1_1,
                                     yend = max(mean_pA_E1,mean_pB_E1)),
                        linetype="dashed",
                        size=0.1)+
  ggplot2::geom_segment(ggplot2::aes(x = X1_0-3.7,
                                     y = mean_pA_E0,
                                     xend = X1_0,
                                     yend = mean_pA_E0),
                        linetype="dashed",
  ...
  ggplot2::geom_segment(ggplot2::aes(x = X1_0-3.7,
                        ...
                        linetype="dashed",
                        size=0.1)+
  annotate("text", label="pA", x = X1_0-4.3, y = mean_pA_E0, size=3)+
  annotate("text", label="pB", x = X1_0-4.3, y = mean_pB_E0, size=3)+
  annotate("text", label="X1,0", x = X1_0, y = -0.0053, size=3)+
  annotate("text", label="X1,1", x = X1_1, y = -0.0053, size=3)


plot_B <- ggplot2::ggplot(data=pA, ggplot2::aes(x=X1, y=X2, z=Perf)) +
  ggplot2::geom_contour(data=pA, colour="midnightblue")+
  ggplot2::geom_contour(data=pB, ggplot2::aes(x=X1, y=X2, z=Perf), colour="orange")+
  ggplot2::coord_cartesian(xlim=c(minX1, maxX1), ylim=c(-5.1, 28), clip="off")+
  ...
  ggplot2::geom_segment(ggplot2::aes(x = X1_1,
                        ...
                        linetype="dashed",
                        size=0.1)+
  annotate("text", label="X1,0", x = X1_0, y = -8.4, size=3)+
  annotate("text", label="X1,1", x = X1_1, y = -8.4, size=3)+
  annotate("text", label="X2,0", x =-6.5, y = X2_0, size=3)+
  annotate("text", label="X2,1", x =-6.5, y = X2_1, size=3)+
  ggplot2::theme(plot.background = ggplot2::element_blank(),
                 panel.grid.major = ggplot2::element_blank(),
                 panel.grid.minor = ggplot2::element_blank(),
                 panel.border = ggplot2::element_blank(),
...
Fig_concept <- ggpubr::ggarrange(plot_A, plot_B, plot_C, plot_D,
                           labels=c("a", "b", "c", "d"),
                           widths = c(1,1.3),
                           hjust=c(-4, -4, -4, -4),
                           vjust=c(2, 2, 2, 2))
ggplot2::ggsave(file=here::here("outputs", "Fig_concept.png"),
                Fig_concept,
                width= 16.6,
                height=16.6/1.5,
                units='cm', 
...
```{r FigConcept}
knitr::include_graphics(here::here("outputs", "Fig_concept.png"))
```
