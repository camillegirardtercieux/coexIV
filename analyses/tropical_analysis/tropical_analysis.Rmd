...
This forest is considered as one of the rare undisturbed tropical forests in the world (Pascal and Pélissier 1996).
...
The data is available here : https://datadryad.org/stash/dataset/doi:10.15146/5xcp-0d46.
...
```{r Plot Paracou, echo=FALSE}
g <- ggplot2::ggplot(data=Paracou, ggplot2::aes(x=Xutm, y=Yutm))+ggplot2::geom_point(size=0.1)
...
There are `r length(unique(Paracou$Sp))` species, `r length(unique(Paracou$idTree))` trees and `r nrow(Paracou)` observations in this dataset.
...
```{r Plot Uppangala, echo=FALSE}
g <- ggplot2::ggplot(data=upsp.dataset, ggplot2::aes(x=Xsite, y=Ysite))+ggplot2::geom_point(size=0.5)
...
```{r Plots BCI, echo=FALSE}
g <- ggplot2::ggplot(data=BCI, ggplot2::aes(x=gx, y=gy))+ggplot2::geom_point(size=0.5, alpha = 0.2)
#Save as png and include in order to make the pdf lighter
...
```{r Abundance diagrams, echo=FALSE}

coexIV::abundance_diagram(Data_Paracou, site = "Paracou")
knitr::include_graphics(here::here("outputs", "tropical_analysis", "figures", "Paracou_abundance.png"))
knitr::include_graphics(here::here("outputs", "tropical_analysis", "figures", "Paracou_abundance_determined.png"))

coexIV::abundance_diagram(Data_Uppangala, site = "Uppangala")
knitr::include_graphics(here::here("outputs", "tropical_analysis", "figures", "Uppangala_abundance.png"))

coexIV::abundance_diagram(Data_BCI, site = "BCI")
knitr::include_graphics(here::here("outputs", "tropical_analysis", "figures", "BCI_abundance.png"))
...
$ln(G_{i,j,t} + 2) = [\beta_0 + b_i + d_j] + \beta_1 \times ln(D_{i,j,t}) + \epsilon_{i,j,t},$ 
...
$b \sim \mathcal{N}(mean=0, var=V_b), iid$

$d \sim \mathcal{N}(mean=0, var=V_d), iid$

$\epsilon_{i,j,t} \sim  \mathcal{N}(mean=0, var=V), iid$
...
```{r Results Uppangala, echo=FALSE}
...
kableExtra::kbl(Results_moran, booktabs = T, )%>%
...
kableExtra::kbl(Results_moran_undisturbed, booktabs = T, )%>%
  kableExtra::kable_styling(latex_options = c("striped", "hold_position", "scale_down"),
                full_width = F)
```
