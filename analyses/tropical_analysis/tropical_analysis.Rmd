...
```{r ChargeParacou, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r PlotParacou, echo=FALSE, fig.cap = "Plots at the Paracou site. Each point is a tree."}
...
2) Compute annualised growth between two censuses and rename all indeterminate species with the same name "Indet_sp". Growth is computed as the difference of DBH (Diameter at Breast Height) between two consecutive censuses, divided by the time period between those two censuses.
...
3) Remove the observations where growth < -2 mm/an or > 100 mm/an and remove the years before the perturbations were performed and the biodiversity plots were added (1985-1991).

```{r RemoveValuesParacou, include=FALSE}
...
4) Compute mean annual growth. Mean growth is computed as the difference of DBH between the first and the last time a tree was measured, divided by the period between those two measures.

```{r MeanGrowthParacou, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
1) Charge the original data.

```{r ChargeUppangala, echo=FALSE}
...
```{r PlotUppangala, echo=FALSE, fig.cap="Plots at Uppangala site. Each point is a tree."}
...
2) Compute annualised growth.

```{r GrowthUppangala, include=FALSE}
...
```{r RemoveValuesUppangala, include=FALSE}
...
4) Compute mean annual growth.

Here are the new columns :

```{r MeanGrowthUppangala, echo=FALSE}
...
2) Remove lines with missing DBH measures and trees with DBH < 10 cm for consistence with the other datasets.
...
```{r PlotBCI, echo=FALSE, fig.cap="The 50 ha plot of the Barro Colorado Island site. Each point is a tree.", warning=FALSE}
...
3) Compute annualised growth.

```{r GrowthBCI, include=FALSE}
...
4) Remove lines with a growth higher than 100 mm/y or inferior to -2 mm/y.

```{r RemoveValuesBCI2, include=FALSE}
...
5) Compute mean annual growth.

```{r MeanGrowthBCI, include=FALSE}
...
```{r HomogeniseDatasets, include=FALSE}
...
```{r AbundanceDiagrams, echo=FALSE, out.width = "50%", fig.show="hold", fig.cap="Abundance diagrams of each site"}
...
We estimate the importance of intraspecific variability compared to interspecific variability.
To do so, we use Stan to run a Bayesian hierarchical model, with a species random effect and an individual random effect on the intercept. In order to approach more classic growth models, and estimate an intraspecific variability within diameter classes, we also added an intercept and a diameter fixed effect.
...
```{r MCMCTraceParacou, echo=FALSE, fig.cap = "Trace of the posterior estimates for Paracou"}
...
```{r MCMCDensityParacou, echo=FALSE, fig.cap = "Density of the posterior estimates for Paracou"}
...
```{r SummaryModelParacou, echo=FALSE}
...
```{r MCMCTraceUppangala, echo=FALSE, fig.cap = "Trace of the posterior estimates for Paracou"}
...
```{r MCMCDensityUppangala, echo=FALSE, fig.cap = "Density of the posterior estimates for Uppangala"}
g <- brms::mcmc_plot(brms_mod, type="dens")
...
```{r SummaryModelUppangala, echo=FALSE}
...
```{r MCMCTraceBCI, echo=FALSE, fig.cap = "Trace of the posterior estimates for BCI"}
...
```{r MCMCDensityBCI, echo=FALSE, fig.cap = "Density of the posterior estimates for BCI"}
g <- brms::mcmc_plot(brms_mod, type="dens")
...
```{r ComputeVarianceProportions, echo=FALSE}
...
```{r PlotsVarianceProportion, echo=FALSE, fig.show="hold", out.width="33%", fig.cap="Proportions of each variance components for the three datasets. The variance of individual effects is in blue, the variance of species effect is in purple and the variance of residuals is yellow"}
...
Overall, a large part of variability is imputable to individual effects, showing a high intraspecific variability in growth in tropical forests, independently from the diameter.
...
```{r ModelsSignificanceMoran, echo=FALSE, message=FALSE, warning=FALSE}
...
In order to compare intra- and interspecific variance, we compute the semivariance of growth of individuals that are less than 100 m apart. We compare the semivariances obtained with conspecifics and heterospecifics with a Mann-Whitney test.
Semivariance is estimated as the mean of the squared difference of the mean growth of all pairs individuals of the focal species which are less than 100 m apart.
Interspecific variance is estimated with the semivariance computed as the mean of the squared difference of the mean growth of all pairs individuals of the containing the focal species and another species which are less than 100 m apart.
...
```{r ResultsVarianceComparison, echo=FALSE}
...
Computing interspecific variance with a sampling of maximum ten indidividuals per species, we obtain a theoretical interspecific variance, not taking the actual proportions of species into account.

```{r ResultsVarianceComparisonSample, echo=FALSE}
...
Sampling individuals within species to estimate interspecific variance did not change the main results.
...
```{r ResultsMoranParacouUndisturbed, echo=FALSE}
